...
def get_samples(task, x, y, n_generate_sample, prompt_sample, stop):
    ...
    elif prompt_sample == 'preact':
        prompt = task.preact_prompt_wrap(x, y, [])
    else:
...
class Node:
    def __init__(self, state, question, parent=None):
        self.state = {'thought': '', 'action': '', 'observation': '', "prediction":""} if state is None else state
        self.parent = parent
    ...
    def __str__(self):
        if "prediction" in self.state:
            return f"Node(depth={self.depth}, value={self.value:.2f}, visits={self.visits}, thought={self.state['thought']}, action={self.state['action']}, observation={self.state['observation']}, prediction={self.state['prediction']})"
        else:
...
def node_trajectory_to_text(node_string):
    ...
    for line in lines:
        try:
            ...
            prediction = line.split(", prediction=")[1].split(", observation=")[0].strip() # so not use prediction to get value
...
def generate_new_states(node, args, task):
    ...
    prompt = generate_prompt(node,args.h)
    ...
    for action in sampled_actions:
        ...
        if args.prompt_sample == 'preact':
            predict_line = next((line.split(":")[1].strip() for line in action.split("\n") if line.startswith("Predicted Feedback") and ":" in line), None)
        ...
        if action_line:
            ...
            if args.prompt_sample == 'preact':
                new_state['prediction'] = predict_line
...
def evaluate_node(node, args, task):
    child_prompts = [generate_prompt(child,args.h) for child in node.children if not child.is_terminal]
...
def generate_prompt(node,h):
    ...
    while node:
        ...
        if node.state["prediction"]:
            if node.depth == 0:
                new_segment.append(f"Predicted Feedback {node.depth}: {node.state['prediction']}")
            else:
                if random.random() < h: 
                    # this is the ablation study. if you want to run this you should 1. run some samples 2. extract the prediction in the samples and save it like
                    #[
                    #     [
                    #         sample 1, depth 1 prediction
                    #         sample 2, depth 1 prediction
                    #         ...
                    #     ],
                    #     ...
                    #     [
                    #         sample 1, depth n prediction
                    #         sample 2, depth n prediction
                    #         ...
                    #     ],
                    # ] in a json file and load it as pred at the beginning of this text. then remove pred = {}
                    
                    pred = {} 
                    pred_info = random.choice(pred[node.depth])
                    new_segment.append(f"Predicted Feedback {node.depth}: {pred_info}")
                    # print(pred_info)
                else:
                    new_segment.append(f"Predicted Feedback {node.depth}: {node.state['prediction']}")
